...
RefOrder = 2; 
w0 =3; zeta = 1;
y = [S.tmp(1),2;zeros(I-1,PlantOrder)];
...
for i = 2:I
   ...
   % end
    ...
    %    
    phip = [phi1(i-1,1);phi2(i-1,1);phi3(i-1,1);y(i-1)];
    %\pmatrix{ \cr}
    %
    zhat(i) = theta(i-1,1:end)*phip;
    
    e(i) = (z(i-1,1) - zhat(i))/(1 + phip'*phip);
   
    u(i) = theta(i-1,1:end)*[w1(i-1);w2(i-1);y(i-1);r(i)];
 

    %Derivatives calculation

    dym   = Wm*ym(i-1,1:end)' + B*r(i);
...
function dy = plant(y1,y2,u,du)
    n = wgn(3,1,1)/2;
    a0 = 3+n(1); a1 = 5+n(2); b0= 7+n(3); b1 =  0;
    %b0 = 1; a1 = 2; a0 = 3; b1 = 0;
    dy(1) = y2;
